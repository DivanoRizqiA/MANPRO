  ...
  async afterRender() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // Pass the analysis result to addCheck so it doesn't have to re-calculate (improves speed & consistency)
        if (response.riskPercentage !== undefined) {
             dataForDb.providedRisk = response.riskPercentage;
        }
        if (response.analysis) {
             dataForDb.providedAnalysis = response.analysis;
        }
