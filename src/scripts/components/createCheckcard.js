export function createCheckCard(check) {
    ...
    // Determine risk category dynamically based on percentage to ensure consistency
    // This fixes cases where old DB data might have incorrect 'riskCategory'
    let riskCategory = "low";
    const riskValue = parseFloat(check.risk || 0);
    ...
    // Fallback usage of check.riskCategory only if needed (optional)
    // But dynamic calculation above is safer for consistency.
    ...
    card.innerHTML = `
      ...
      <div style="display:flex; justify-content:flex-end; margin-top:14px;">
        <button class="clear-history-button" data-check-id="${check.id || check._id}" title="Hapus riwayat ini">
